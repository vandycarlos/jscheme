...
/**
 * A primitive is a procedure that is defined as part of the Scheme report, and is implemented in
 * Java code.
 *
...
public class Primitive extends Procedure {
  ...
  public Object apply(Scheme interp, Object args) {
    ...
    switch (idNumber) {
      ...
      case STRINGTOLIST:
        Pair result = null;
        char[] str2 = str(x);
        for (int i = str2.length - 1; i >= 0; i--) result = cons(chr(str2[i]), result);
        return result;
      case LISTTOSTRING:
      ...
      case FORCE:
        return (!(x instanceof Procedure)) ? x : proc(x).apply(interp, null);
      case MACROEXPAND:
  ...
  /**
   * Return <0 if x is alphabetically first, >0 if y is first, 0 if same. Case insensitive iff ci is
   * true. Error if not both chars. *
   */
  ...
  public static int stringCompare(Object x, Object y, boolean ci) {
    if (x instanceof char[] && y instanceof char[]) {
      ...
      for (int i = 0; i < xc.length; i++) {
        int diff =
            (!ci) ? xc[i] - yc[i] : Character.toUpperCase(xc[i]) - Character.toUpperCase(yc[i]);
  ...
  static boolean isList(Object x) {
    ...
    for (; ; ) {
      ...
      if (slow == rest(fast) || !(fast instanceof Pair) || !(slow instanceof Pair)) return false;
      slow = rest(slow);
      fast = rest(fast);
      if (fast == null) return true;
  ...
  /**
   * Map proc over a list of lists of args, in the given interpreter. If result is non-null,
   * accumulate the results of each call there and return that at the end. Otherwise, just return
   * null. *
   */
