...
public class JavaMethod extends Procedure {
  ...
  public JavaMethod(String methodName, Object targetClassName, Object argClassNames) {
  ...
  /**
   * Convert a list of Objects into an array. Peek at the argClasses array to see what's expected.
   * That enables us to convert between Double and Integer, something Java won't do automatically. *
   */
  public Object[] toArray(Object args) {
    int n = length(args);
    int diff = n - argClasses.length;
    if (diff != 0)
      error(Math.abs(diff) + " too " + ((diff > 0) ? "many" : "few") + " args to " + name);
    Object[] array = new Object[n];
    for (int i = 0; i < n && i < argClasses.length; i++) {
      if (argClasses[i] == java.lang.Integer.TYPE) array[i] = new Integer((int) num(first(args)));
      else array[i] = first(args);
      args = rest(args);
    }
    return array;
  }
