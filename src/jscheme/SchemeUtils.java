...
/**
 * @author Peter Norvig, peter@norvig.com http://www.norvig.com Copyright 1998 Peter Norvig, see
...
public abstract class SchemeUtils {
  ...
  /**
   * Coerces a Scheme object to a Scheme input port, which is an InputPort. If the argument is null,
   * returns interpreter.input. *
   */
  ...
  /**
   * Coerces a Scheme object to a Scheme input port, which is a PrintWriter. If the argument is
   * null, returns System.out. *
   */
  ...
  public static Pair vectorToList(Object x) {
    if (x instanceof Object[]) {
      Object[] vec = (Object[]) x;
      Pair result = null;
      for (int i = vec.length - 1; i >= 0; i--) result = cons(vec[i], result);
      return result;
    } else {
  ...
  /**
   * Convert a Scheme object to its printed representation, as a java String (not a Scheme string).
   * If quoted is true, use "str" and #\c, otherwise use str and c. You need to pass in a
   * StringBuffer that is used to accumulate the results. (If the interface didn't work that way,
   * the system would use lots of little internal StringBuffers. But note that you can still call
  ...
  /**
   * Convert x to a Java String giving its external representation. Strings and characters are
   * quoted. *
   */
  ...
  /**
   * Convert x to a Java String giving its external representation. Strings and characters are
   * quoted iff <tt>quoted</tt> is true.. *
   */
