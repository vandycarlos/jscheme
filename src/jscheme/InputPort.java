...
public class InputPort extends SchemeUtils {
  ...
  /**
   * Peek at and return the next Scheme character as an int, -1 for EOF. However, don't consume the
   * character. *
   */
  public int peekCh() {
    try {
  ...
  public Object read() {
    try {
      ...
      else if (token == "`") return list("quasiquote", read());
      else if (token == ",") return list("unquote", read());
      else if (token == ",@") return list("unquote-splicing", read());
      else return token;
    } catch (IOException e) {
      warn("On input, exception: " + e);
      return EOF;
