...
/**
 * This class represents a Scheme interpreter. See http://www.norvig.com/jscheme.html for more
 * documentation. This is version 1.4.
 *
...
public class Scheme extends SchemeUtils {
  ...
  /**
   * Create a new Scheme interpreter, passing in the command line args as files to load, and then
   * enter a read eval write loop. *
   */
  ...
  /**
   * Prompt, read, eval, and write the result. Also sets up a catch for any RuntimeExceptions
   * encountered. *
   */
      ...
      if (x instanceof String) { // VARIABLE
        ...
        if (fn == "quote") { // QUOTE
          ...
          if (first(args) instanceof Pair)
            return env.define(
                first(first(args)), eval(cons("lambda", cons(rest(first(args)), rest(args))), env));
  ...
  Pair evalList(Object list, Environment env) {
    if (list == null) return null;
      ...
      return null;
    } else return cons(eval(first(list), env), evalList(rest(list), env));
  }

  /**
   * Reduce a cond expression to some code which, when evaluated, gives the value of the cond
   * expression. We do it that way to maintain tail recursion. *
   */
  Object reduceCond(Object clauses, Environment env) {
    ...
    for (; ; ) {
      ...
      if (first(clause) == "else" || truth(result = eval(first(clause), env)))
        ...
        else if (second(clause) == "=>") return list(third(clause), list("quote", result));
